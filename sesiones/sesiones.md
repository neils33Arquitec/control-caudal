...
| `src/lib/PIController.h` | Clase `PIController` con métodos: `compute()`, `reset()`, `setGains()`, `getIntegral()`. |
| `src/lib/PIController.cpp` | Implementación: PI discreto por Backward Euler, anti-windup por clamping con saturación condicional del integrador. |
| `src/include/config.h` | Constantes del sistema: pines (2, 7, 8, 9, A0), Kp=2.0, Ki=0.5, Ts=0.1s, 5880 pulsos/L, LCD 0x27. |
...
| `src/include/config.h` | Eliminadas macros no utilizadas (`CAUDAL_MAXIMO`, `PIN_LCD_SDA`, `PIN_LCD_SCL`). |
